...
Este código pertenece al shield de PWFusion, este se encarga de recopilar las mediciones de temperatura del aire que realizan los termopares tipo T:
...
El siguiente código gestiona el Adafruit PT1000 RTD-MAX31865, encargado de convertir las señales analógicas del TPF1/E-20 PT1000 a valores digitales, permitiendo obtener las mediciones de la temperatura radiante:
...
El siguiente código se encarga de gestionar el Wind Sensor Rev. P6, permitiendo obtener las mediciones de la velocidad del viento:
...
try:
    while True:
        # Obtiene las mediciones del SCD30
        ...
        # Obtiene el valor de iluminancia
        camera_data = illuminance_data()
        # Publica el valor de iluminancia en ThingsBoard
        client.publish('v1/devices/me/telemetry', json.dumps(camera_data), 1)
...
Este código se encarga de capturar y almacenar las mediciones de humedad relativa, concentración de CO<sub>2</sub> y temperatura ambiente proporcionadas por el sensor SCD30:
```{python}
#| eval: false
#| code-overflow: wrap
import time
...
A continuación, se presentan los códigos utilizados para la captura y análisis de sonido. Estos códigos permiten grabar el audio, extraer parámetros clave de la señal y convertirlos a valores en decibeles (dB).

El siguiente código graba un archivo de audio en formato WAV durante 15 segundos a una frecuencia de muestreo de 44.1 kHz:
...
Posteriormente este código analiza el archivo de audio y extrae la amplitud máxima de la señal, almacenándola en un archivo de texto:
...
Similar al anterior, este código obtiene la amplitud mínima del audio y la guarda en un archivo de texto:
...
#### OV5647 Ultra Wide Angle

El siguiente programa se encarga de obtener el valor total de iluminancia mediante la generación de un mapa de iluminancia. En primer lugar, el código captura múltiples fotografías a diferentes velocidades de obturación, lo que permite cubrir un amplio rango de exposiciones. Posteriormente, se fusionan estas imágenes para crear una imagen HDR, que contiene la mayor cantidad de información luminosa posible. A continuación, a la imagen HDR se le aplica un false color para resaltar la distribución de la iluminancia en las distintas superficies de la escena. Utilizando herramientas del software Radiance, se realiza un cálculo de integral sobre los valores de luminancia de la imagen, lo que permite determinar el valor total de iluminancia. Finalmente, este valor se almacena en un archivo .txt para su posterior análisis:
```{python}
#| eval: false
#| code-overflow: wrap
#!/bin/bash
...
Posteriormente este código define una función que se encarga de leer el valor total de iluminancia a partir de un archivo .txt previamente generado por el proceso de creación del mapa de iluminancia. La función abre el archivo ubicado en la ruta especificada, extrae el valor numérico contenido en él, lo convierte a tipo float y lo retorna:
```{python}
#| eval: false
#| code-overflow: wrap
import numpy as np
...
def illuminance_data():
    """
    Función para obtener el valor total de iluminancia desde el archivo .txt generado por el proceso de generación del mapa de iluminancia.
    
    Retorna:
        float: Valor total de iluminancia.
    """
    file_path = '/home/hdeza/Luminance/illuminance.txt' 
    with open(file_path, 'r', encoding='utf-8') as f:
        value_str = f.read().strip()
    illum_value = float(value_str)
    return illum_value
```
